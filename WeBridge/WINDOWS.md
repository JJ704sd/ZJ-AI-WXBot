...
产品路线为数据库副本接收、独立 Windows Hook 接口发送。在“Hook 发送”中主动检查模块，查看实际微信版本、架构、模块 SHA-256 摘要和阻断原因。未配置本机桥接、当前模块未适配、账号不一致或校验未通过时，发送按钮保持禁用；不能仅凭数据库可读认定 Hook 可发送。

Hook 桥接使用本机回环 HTTP 接口，其地址、凭证和版本配置由服务端管理，不在网页提交。工作台不负责加载或注入 Hook 模块，启动器也不会因此注入微信。实际版本适配与真实发送结果以当前状态及单独验收记录为准。

就绪后，手动发送只允许当前账号已选入读取范围的会话：选择目标、填写文本，预检后再次核对服务端绑定的目标与正文，再确认发送一次。数据源身份或目标变化会使旧确认失效。此入口不启动定时任务或自动回复，也不发送附件。

发送后区分等待确认、服务端已接受、本机记录已核对和结果未知。工作台最多每 3 秒查询一次结果，共 10 次；达到上限显示待核实，不自动重发。发送接口返回、服务器接受或本机消息记录均不等于收件端已读或已送达。超时和结果未知时应先在微信核对，不能直接重复发送。
