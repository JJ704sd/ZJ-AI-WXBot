...
- The archive has **no project LICENSE** for its own Python sources. This PoC **reimplements** UTF-8/Zstandard decoding, `Msg_<md5>` table candidates, and multi-shard discovery as restricted functions. It does not copy attachment files, `.venv`, databases, key extractors, or decryptors.
- Page decryption is **not** implemented (spec §12: do not add it unless encrypted samples are in scope).
