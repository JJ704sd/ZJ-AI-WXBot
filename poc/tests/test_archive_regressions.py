...
def test_two_shards_merge_without_overwrite(tmp_path: Path):
    ...
    assert all(event.is_historical for event in first)
...
def test_openim_whitelist_kept_and_non_whitelist_dropped(tmp_path: Path):
    ...
    write_db(
        ...
        CONVERSATION,
        [{"local_id": 1, "text": "keep-openim"}],
        extra_conversations=[OTHER_GROUP],
    )
    ...
    conn.close()
    store = Store(tmp_path / "state.sqlite")
    events = SqlitePlainReader(config, store).read_new_messages()
...
def test_same_body_two_native_ids(tmp_path: Path):
    ...
    events = SqlitePlainReader(config, Store(tmp_path / "s.sqlite")).read_new_messages()
    assert len(events) == 2
...
def test_does_not_select_first_discovered_account(tmp_path: Path):
    ...
    events = SqlitePlainReader(make_config(tmp_path), Store(tmp_path / "s.sqlite")).read_new_messages()
